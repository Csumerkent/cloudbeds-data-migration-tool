...
const INSTRUCTION_ROWS: string[][] = [
  ...
  ['- The value may be simplified; the app similarity-matches it against the available Cloudbeds sources.'],
  ['- Matching order: case-insensitive exact → normalized exact → contains/similarity (closest valid match wins).'],
  ['- If Source Code is left BLANK, the configured default source is used:'],
  ['    • past reservations  → configured "past" default source (e.g. FORMERPMS)'],
  ['    • future reservations → configured "future" default source (e.g. Direct - Hotel)'],
  ['- Defaults are configured on the Source Configuration page.'],
  [],
  ['Rate Code'],
  ['- Enter the rate plan public name if applicable. The value may be simplified;'],
  ['  the app similarity-matches it against the configured rates for the row\'s room type.'],
  ['- Matching tolerates dash / no-dash / spacing differences:'],
  ['    directro / direct-ro / direct ro → Direct-RO'],
  ['    walkin / walk-in / walk in       → Walkin'],
  ['    formerpms                        → FORMERPMS'],
  ['- If Rate Code is left BLANK, the configured default rate is used:'],
  ['    • past reservations  → configured default historical rate (e.g. FORMERPMS)'],
  ['    • future reservations → configured default future rate (e.g. Walkin)'],
  ...
  ['- sendEmailConfirmation is ALWAYS sent as false; guests never receive a confirmation email during migration.'],
  [],
  ['Zip'],
  ['- The Zip column is ignored. It is intentionally left blank / omitted from every payload.'],
  [],
  ...
  ['- Adult: 1'],
  ['- Child: 0'],
  ['- Source Code: configured past/future default (see above)'],
  ['- Rate Code: configured past/future default (see above)'],
  [],
  ['Payment Note'],
